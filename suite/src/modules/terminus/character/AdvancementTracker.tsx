...
const DICE_ORDER: Die[] = ['d4', 'd6', 'd8', 'd10', 'd12'] as const;

interface SkillAdvancementRowProps {
  skill: 'Force' | 'Agility' | 'Willpower';
  thresholdName: string;
  currentDie: Die;
  simulatedAP: number;
  expandedSkill: string | null;
  onToggleExpanded: (skill: string) => void;
  onAdvance: (skill: 'Force' | 'Agility' | 'Willpower') => void;
}

function SkillAdvancementRow({
  skill,
  thresholdName,
  currentDie,
  simulatedAP,
  ...
  const currentIndex = DICE_ORDER.indexOf(currentDie);
  ...
  return (
    <div className="border border-slate-700 rounded p-4 space-y-3">
      <div
        ...
        onClick={() => onToggleExpanded(skill)}
      >
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2 mt-2">
              {[
                ...
                const fromIndex = DICE_ORDER.indexOf(from as Die);
    ...
    <div className="max-w-4xl mx-auto bg-slate-900 border border-slate-700 rounded-lg p-6 space-y-6">
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <SkillAdvancementRow
            skill="Force"
            thresholdName="Endure"
            currentDie={skills.Force}
            simulatedAP={simulatedAP}
            expandedSkill={expandedSkill}
            onToggleExpanded={(skill) => setExpandedSkill(expandedSkill === skill ? null : skill)}
            onAdvance={handleAdvanceSkill}
          />
          <SkillAdvancementRow
            skill="Agility"
            thresholdName="Avoid"
            currentDie={skills.Agility}
            simulatedAP={simulatedAP}
            expandedSkill={expandedSkill}
            onToggleExpanded={(skill) => setExpandedSkill(expandedSkill === skill ? null : skill)}
            onAdvance={handleAdvanceSkill}
