/* ── Terminus Setting Pack — AI generators ── */

import { completeJSON, isRecord } from '../../../services/aiService';
...
const SCENE_SYSTEM_PROMPT = `You are the Coherence Engine for the Terminus RPG under the Coherence System. You generate "GWSD" Scene Cards.
...
export async function generateSceneFromPrompt(
  ...
  const userPrompt = `Generate a Terminus RPG GWSD scene based on this prompt: "${request.prompt}"\nAdventure context: ${request.adventure || 'None'}\nAct context: ${request.act || 'None'}`;
  const parsed = await completeJSON({ systemPrompt: SCENE_SYSTEM_PROMPT, userPrompt, apiKey, baseUrl, model });
...
const NAME_SYSTEM_PROMPT = `You are the Nomenclator Engine for the Terminus RPG. You generate "GeneratedName" data records.
...
Ensure all generated names are extremely high-quality and directly usable at a Tringad playtest table. Make sure they sound evocative, tragic, and civic-bureaucratic.`;
  ...
  const parsed = await completeJSON({ systemPrompt: NAME_SYSTEM_PROMPT, userPrompt, apiKey, baseUrl, model });
...
const ADVENTURE_SYSTEM_PROMPT = `You are the Adventure Architect for the Terminus RPG under the Coherence System.
...
Ensure the generated NPCs have active wills and are anchored in social standings. Ensure the encounters include both triggered complications and fallback stuck cues. DO NOT use generic placeholders or write "TBD". Complete every field with rich, high-quality, atmospheric, and playable Terminus narrative.`;
  ...
  const parsed = await completeJSON({ systemPrompt: ADVENTURE_SYSTEM_PROMPT, userPrompt, apiKey, baseUrl, model });
