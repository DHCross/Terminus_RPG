...
- no hit/miss opposed-roll combat
...
An action is not a request for permission to matter. The action matters. The question is how the pressure is taken, routed, softened, redirected, or paid for.

Terminus may use **two active rolls**, but it should not be described as ordinary opposed attack-vs-defense combat.

The key distinction:

```text
Not this:
I roll to hit. You roll or use a number to stop me. If I fail, nothing happens.

This:
I create pressure. You choose how you take it. We roll to see who controls the terms of that exchange. Either way, the scene changes.
```

## Core Exchange

```text
1. The acting side declares intent.
2. The acting side chooses a Skill: Force, Agility, or Willpower.
3. The responding side chooses a Threshold response: Endure, Avoid, or Exert.
4. Both roll.
5. The higher roll controls the terms of the exchange.
6. The action resolves through Impact, Vector, Threshold loss, position change, exertion, or scene change.
```
...
There is no “roll to see whether your action exists.”

The response roll is not a passive block. It is an active routing choice.

## Pressure Roll and Response Roll

Use this language at the table and in software tools:

| Role | Roll | Meaning |
|---|---|---|
| Pressure Roll | Force, Agility, or Willpower | How the acting side applies pressure |
| Response Roll | Endure, Avoid, or Exert | How the responding side routes that pressure |

Avoid UI labels like **Attacker** and **Defender** when possible. They push the system back toward hit/miss combat.
...
- Spends Exert
- Loses a circle
- Scene pressure rises

## Skill / Threshold Pairing

| Skill | Linked Threshold |
|---|---|
| Force | Endure |
| Agility | Avoid |
| Willpower | Exert |

The responding side is not required to choose the linked Threshold. A target may answer Force with Avoid, or Agility with Exert, if the fiction supports it.

The linked pair simply tells you which Threshold die comes from which Skill die.

## What Control Means

Winning the roll does not always mean “damage.”

Winning the roll means you control how the exchange resolves.

Examples:

- You impose your weapon Impact.
- You trigger a Vector.
- You force movement.
- You keep position.
- You avoid losing a Threshold circle.
- You make the other side pay with Exert.
- You keep a Working intact.
- You cause the scene pressure to rise.
- You decide which consequence happens first.

The Guide should resolve the exchange through the current Scene Card.

## No Dead Results

No result should mean “nothing happens.”

If the Pressure Roll wins, the acting side gets the cleaner expression of intent.

If the Response Roll wins, the responding side does not erase the action from existence. They control the cost, angle, timing, or route of the pressure.

Examples:

```text
Force vs Avoid
The Breaker swings the hammer. The Shade chooses Avoid and wins.
The hammer still crashes into the scene, but the Shade gives ground, slips behind the signal post, and marks one Avoid circle instead of taking Endure loss.

Agility vs Endure
The Rival lunges through a gap. The Warden chooses Endure and wins.
The Rival reaches the line, but the Warden absorbs the rush, holds position, and forces the Rival to stop short.

Willpower vs Exert
The Seeker names the repeated step. The trapped pedestrian's loop pushes back through Exert.
If the Seeker wins, the pedestrian hears the name. If the loop wins, the Seeker still perceives the pattern, but must mark Exert or let scene pressure rise.
```
...
Use Avoid when you route pressure through movement, timing, distance, or position.
...
**On a tie, neither side cleanly controls the exchange. The scene pressure rises, or the Guide advances Drift.**

This keeps ties from becoming dead air.

Optional simpler rule:
...
6. Did the response choice feel like pressure routing rather than a passive defense?
...
Conflict:
Pressure side rolls Skill.
Response side chooses Threshold and rolls.
Higher roll controls the terms of the exchange.
The response roll routes pressure; it does not erase the action.
No target number.
No to-hit roll.
No passive defense.
No dead result.
