...
function getSuiteShellUrl(): string {
  const params = new URLSearchParams(window.location.search);
  const suiteMode = params.get('suiteMode') === 'packaged' ? 'packaged' : 'dev';
  const suiteConfigUrl = params.get('suiteConfigUrl');
  const host = window.location.hostname || 'localhost';
  const baseUrl = suiteMode === 'packaged'
    ? `${window.location.origin}/`
    : `http://${host}:3000/`;
  const url = new URL(baseUrl);
  url.searchParams.set('suiteMode', suiteMode);
  if (suiteConfigUrl) {
    url.searchParams.set('suiteConfigUrl', suiteConfigUrl);
  }
  return url.toString();
}
...
export default function App() {
  ...
  const handleBackToSuite = useCallback(() => {
    window.location.href = getSuiteShellUrl();
  }, []);
      ...
      >
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          ...
          <button
            ...
            title="Return to TRPG Workbench Tools Suite"
          >
            ← Back to TRPG Workbench Tools Suite
          </button>
